...
<DIV class=box>
<H4>管理员登录</H4>
<FORM id=user method=post name=user action="${pageContext.request.contextPath}/adminCheck">
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
  ...
  <TR>
    ...
    <TD><!-- <input type="password" class="text" name="password" /> --><INPUT 
      id=user_password class=text type=password name=password> </TD></TR><!--
						...
						--></TBODY></TABLE>
<DIV class=buttons> <INPUT  value="登陆" type="submit"> <INPUT onclick="goRegs()" value=注册 type=button><br>
...
<script>
  function goRegs() {
    window.location.href = "${pageContext.request.contextPath}/admin/regs.jsp";
  }
</script>
<script>
